...
} from "../redux/events/thunk/comment-thunk";
...
const EventDetails = () => {
  ...
  console.log(eventId);
  const { comments, isLoading, isError, errorMessage, isPageLoading } =
    useSelector((state) => state.comments);
  const { eventDetails, isevPageLoading } = useSelector(
    (state) => state.events
  );
  DynamicTitle(eventDetails?.title || "Event");
  ...
  if (isPageLoading && isevPageLoading && !token) return <FormLoader />;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        ...
        {isLoading && (
          <p className="text-sm text-gray-400">Posting comment...</p>
        )}
